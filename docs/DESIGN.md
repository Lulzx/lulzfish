...
- `graph_benchmark` now reports graph relation counts plus throughput for graph evaluation, feature extraction, full graph rebuilds, and make/unmake graph-update cost. The current extended relation implementation deliberately falls back to full relation rebuilds during make/unmake while the richer relation types and feature vector settle; this is correct but not the final incremental accumulator needed for the Graph NNUE thesis.
...
This gives us a clean correctness foundation for the next stage: reintroduce optimized attack generation safely, harden search output, and replace graph full-rebuild-on-move with measured incremental feature accumulators.
