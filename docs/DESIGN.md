...
- Search has alpha-beta, iterative deepening at the root, a bounded check extension, quiescence with a small quiet-check budget, TT bounds/hash move ordering, null move, SEE, history, killers, and root best-move reporting. It still needs PV output and proper match infrastructure before playing strength claims mean much.
- `search_regression` covers a few cheap strength sanity checks so obvious opening/tactical regressions fail in the build-test loop. `tools/lulzfish_match.py` provides lightweight bulletchess-based self-play and Stockfish smoke matches until a cutechess/SPRT harness is added.
