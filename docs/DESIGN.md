...
- Search has alpha-beta, iterative deepening at the root, a bounded check extension, quiescence with a small quiet-check budget, TT bounds/hash move ordering, repetition scoring from `Position` key history, null move, SEE, history, killers, and root best-move reporting. It still needs PV output and proper match infrastructure before playing strength claims mean much.
